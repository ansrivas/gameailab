...
class Pong(pygame.sprite.Sprite):
    ...
    def update(self, changeDirection, screen):
        if(changeDirection == self.speed):
            self.theta += self.speed
        elif(changeDirection == -(self.speed)):
            if(self.theta >= 0):
                self.theta -= self.speed
            else:
...
def game(screen):
    ...
    pointCount = 0
    ballx1 = bally1 = ballx2 = bally2 = 0.0
    ballAngle = 0
    
    # Predicted Bat points and angle
    batAngle = 0.0
    predictedBatPointX1 = predictedBatPointY1 = predictedBatPointX2 = predictedBatPointY2 = 0.0
    
    # Find the best fit among the the 2 points
    predictedBatAngle = predictedBatPointX = predictedBatPointY = 0.0
        ...
        for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_a:
                        WolverchangeDirection = main.wolverPong.speed
                ...
                if (ignoreCollide==0):
                    ...
                    if(main.MULTIPLAYER):
                        if(player == -1):
                            ...
                            batAngle = np.deg2rad(main.rayPong.angle)
                            
                            # Get any point on ball's trajectory, In our case "4"
                            if(pointCount == 4):
                                ballAngle = main.fireBall.angle 
                                
                                ballx1,bally1 = main.fireBall.rect.center
                            
                            # Get another point on ball's trajectory, In our case "5"    
                            if(pointCount == 5):
                                
                                ballx2,bally2 = main.fireBall.rect.center
                                                           
                                # Predict bat coordinates according to ball angle
                                predictedBatPointX1,predictedBatPointY1,predictedBatPointX2,predictedBatPointY2 = intersect.lineCircleIntersect(ballx1*1.0,bally1*1.0, \
                                                                                                                                                ballx2*1.0, bally2*1.0, \
                                                                                                                                                main.fireBall.angle, radius, \
                                                                                                                                                SCREEN_W/2, SCREEN_H/2)
                                # Get the Best Fit Bat Point and Angle
                                predictedBatAngle,predictedBatPointX,predictedBatPointY = predictAngle.findBestFitAngle(ballAngle,predictedBatPointX1,predictedBatPointY1, \
                                                                                                                        predictedBatPointX2,predictedBatPointY2,SCREEN_W/2, \
                                                                                                                        SCREEN_H/2, radius)
                                predictedBatAngle = int(np.rad2deg(predictedBatAngle))
                                predictedBatPointX = int(predictedBatPointX)
                                predictedBatPointY = int(predictedBatPointY)
                                
                            pointCount += 1
                            
                            # After getting the Best Fit , move Bat
                            if(pointCount >5):
                                batAngle = int(np.rad2deg(batAngle))
                               
                                if(batAngle <> predictedBatAngle):
                                    
                                    if((batAngle+1) <> predictedBatAngle and (batAngle-1) <> predictedBatAngle):
                                        if(batAngle > predictedBatAngle):
                                            RaychangeDirection = -(main.rayPong.speed)
                                            main.rayPong.update(RaychangeDirection,screen)
                                        else:    
                                            RaychangeDirection = main.rayPong.speed
                                            main.rayPong.update(RaychangeDirection,screen)
                                        ...
                                        main.rayPong.update(RaychangeDirection,screen)
                                        if(debug):
                                            print "Bat Position : ",main.rayPong.rect.x,main.rayPong.rect.y
                                            print "Bat Center Position : ",main.rayPong.rect.center
                                else:
                                    RaychangeDirection = 0
                                    main.rayPong.update(RaychangeDirection,screen)
                                    if(debug):
